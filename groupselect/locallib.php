...
function get_students($groupid){
       global $DB;
       $query = "SELECT x.id, CONCAT(x.firstname, ' ', x.lastname) as fullname FROM {user} x JOIN {role_assignments} y ON y.userid = x.id WHERE y.roleid = ? AND y.userid NOT IN ( SELECT userid FROM {groups_members} WHERE groupid = ?)";
              
              $users = $DB->get_records_sql($query,array('5',$groupid));
              $select= "<form style='margin-top:5px;margin-left:-45px;' method='post'><select name='select1' >";
              $select.= '  <option value="" selected disabled hidden>Select Student</option>';
